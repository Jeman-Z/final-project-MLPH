...
```{r}
## remove those variables that do not show significant associations with obesity
train_data <- train_data[, !names(train_data) %in% c("smoke","Age","water")]
test_data <- test_data[, !names(test_data) %in% c("smoke","Age","water")]
```
...
library(gbm)
set.seed(0)
boost_model <- gbm(obesity ~ .,data = train_data, distribution = "multinomial", n.trees = 5000, interaction.depth = 2,cv.folds = 10,n.cores = 1)
summary(boost_model)
```
...
yhat.boost <- levels(data$obesity)[apply(yprob.boost, 1, which.max)]
table(yhat.boost,train_data$obesity)
```

```{r}
(381+1193)/(381+1193+49+67)
```
...
yhat.boost <- levels(data$obesity)[apply(yprob.boost, 1, which.max)]
table(yhat.boost,test_data$obesity)
```
```{r}
(84+278)/(84+27+32+278)
```
...
boost_probs <- predict(boost_model, newdata = test_data, n.trees = 5000, type = "response")[,"Obesity",]
...
mdata$meal <- scale(mdata$meal)

# Splitting data into training and test sets
set.seed(123) # for reproducibility
train_indices <- sample(1:nrow(mdata), size = 0.8 * nrow(mdata))
